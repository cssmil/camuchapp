...
export function EditProductModal({ isOpen, onClose, onProductUpdated, producto }: EditProductModalProps) {
  ...
  useEffect(() => {
    if (producto) {
      ...
      setProveedorId(producto.proveedor?.id || null);
      setStockMinimo(producto.stock_minimo);
      setFechaVencimiento(producto.fecha_vencimiento ? new Date(producto.fecha_vencimiento) : undefined);
    }
  }, [producto]);
